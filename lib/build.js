    ...
    tarfs.pack(pathentry, {
      ignore: function(name) {
        ...
        for (var i = 0; i < ignores.length; i++) {
          if (!ignores[i] || ignores[i] === '') continue;

          var exception = (ignores[i].substring(0, 1) == '!'),
              pattern = (exception) ? ignores[i].substring(1) : ignores[i];

          if (exception && minimatch(relativeName, pattern)) {
            ignore = false;
          }
          else {
            ignore = ignore || minimatch(relativeName, pattern);
          }
        }

        if (ignore) grunt.log.debug('Ignoring file [' + relativeName +']');
