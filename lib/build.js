...
var buildCommands = function(grunt) {
  ...
  // Build image with tag and its data.
  // Call callback when done or error.
  var build = function(data, tag, callback) {
    docker.buildImage(data, {t: tag}, function(err, stream) {
      grunt.log.subhead('Building image [' + tag + ']');
  ...
  var process = function(image, callback) {
    ...
    if (fileExt) {
      build(content, image.tag, callback);
    } 
    else {
      var pack = tar.pack();
  ...
  async.doWhilst(
    function (callback) {
      i ++;
      process(options.images[i-1], callback);
    },
    function () { return i < options.images.length; },
    function (e) {
