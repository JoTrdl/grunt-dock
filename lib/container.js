    ...
    async.waterfall([
      ...
      function(cb) {
        docker.listContainers({
          ...
          for (var c = 0; c < containers.length; c++) {
            if (containers[c].Image.indexOf(utils.qualifiedImageName(tag, options.registry, null) + ':') === 0) {
              // If the current container is running
              if (containers[c].Status.toLowerCase().indexOf("up ") === 0) {
      ...
      function(container, cb) {
        if (container) {
          ...
          var opts = confImage && confImage.options && confImage.options.create || {};
          opts.Image = utils.qualifiedImageName(tag, options.registry, confImage && confImage.tag);
 ...
 * @param  {Object}   options The Grunt options
 * @param  {Function} done    The done function to call when finished
 * @param  {String}   tag     The image tag to process with or null
 */
